...
sijill/
  ...
  scripts/
    ...
    tamper.sh        # demo helper: backup | alter | forge | restore
  policy.yaml
  tests/
  README.md
```
...
| model_id | text | Model name as requested |
| model_digest | text | Content digest reported by the runtime |
| node_id | text | From policy file |
| node_region | text | Current region of the node |
| policy_id | text | From policy file |
| policy_hash | text | SHA-256 of the policy file bytes in force |
...
One page, `dashboard/index.html`. It is the only browser window on stage, beside one terminal. Polls every second. Five regions, top to bottom:

1. **Header** — node id, current region (shown red when outside the residency allowlist), total record count.
2. **Policy strip** — each rule with its current mode. **Display only.** Modes are changed by editing `policy.yaml` in the terminal and calling reload. No editing controls on the page.
3. **Send panel** — two preset buttons calling the proxy: a plain question, and one containing "Emirates ID". A region selector calling the admin region endpoint.
4. **Ledger** — newest first: seq, decision, rules hit, first 12 characters of `record_hash`. `allow` plain, `flag` amber, `block` red, `policy_change` grey.
5. **Verifier bar** — last result from `/api/verify`, re-run every 2 seconds. Large monospace text: green on PASS, red on FAIL naming the seq and reason. A Generate report button beside it.

**The console must never offer any way to alter records.** Tampering happens only in the terminal, outside the application. That separation is what makes the catch credible to the judges.

## Tamper script

`scripts/tamper.sh`, run in the terminal on stage. Each command prints one short, legible line.

| Command | Does |
| --- | --- |
| `tamper.sh backup` | Copies `sijill.db` aside. Run before the demo |
| `tamper.sh alter 17` | Changes `output_hash` of seq 17 directly with sqlite3. Verifier should fail: content altered |
| `tamper.sh forge 17` | Changes seq 17's content and recomputes its `record_hash` with the canonical algorithm, without the private key. Verifier should fail: signature invalid |
| `tamper.sh restore` | Restores the backup. Verifier returns to PASS |

## Fallback recording

As soon as the full demo runs end to end once, record it before any polish. If anything fails on stage, the presenter switches to the recording.
